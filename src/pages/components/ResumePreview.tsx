...
const ResumePreview: React.FC<ResumePreviewProps> = ({ data }) => {
  // Create a URL for the image file if available, or fallback to undefined
  const profileImage = data?.image ? URL.createObjectURL(data.image) : undefined;

  // Generate fallback initials if no image is provided
  const fallbackText = !data?.image ? getInitials(data.name) : null;

  // Reference for the resume content to print
    ...
    <div>
      <div ref={resumeRef} className="max-w-4xl mx-auto p-4 bg-white rounded-xl shadow-2xl flex flex-col sm:flex-row">
        ...
        <div className="w-full sm:w-1/3 bg-gradient-to-tl from-indigo-500 to-purple-600 p-6 rounded-l-xl flex flex-col items-center text-white">
          ...
          <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-xl mb-4 flex items-center justify-center bg-gray-300">
            {fallbackText ? (
              <span className="text-xl text-white font-bold">{fallbackText}</span>
            ) : (
              <Image
                src={profileImage || '/path/to/default-image.jpg'}
                alt="Profile"
                className="w-full h-full rounded-full"
                width={128} // Define width and height to avoid layout shift
                height={128}
              />
            )}
          </div>
